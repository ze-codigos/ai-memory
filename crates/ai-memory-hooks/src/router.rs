...
/// ze-codigos fork: heading of the topic line (see `BRIEF_TOPICS_LIMIT`).
const BRIEF_TOPICS_HEADER: &str = "\n**Topics this project's memory covers** \
...
/// ze-codigos fork: render the topic line — the subjects a project's durable
/// pages cover, as one comma-separated line — shrinking to the topics that
/// fit `budget`, else nothing. An untagged project gets no empty scaffold.
fn render_brief_topics_section(topics: &[String], budget: usize) -> String {
    if topics.is_empty() || budget <= BRIEF_TOPICS_HEADER.len() {
        return String::new();
    }
    let mut out = String::from(BRIEF_TOPICS_HEADER);
    let mut listed = 0;
    for topic in topics {
        let sep = if listed == 0 { "" } else { ", " };
        // The closing newline is paid for up front so the line always ends.
        if out.len() + sep.len() + topic.len() + 1 > budget {
            break;
        }
        out.push_str(sep);
        out.push_str(topic);
...
fn render_session_brief(
    ...
    let pointer_desire = render_brief_omitted_section(core, usize::MAX).len()
        + render_brief_topics_section(topics, usize::MAX).len()
    ...
    // ze-codigos fork: the topic line sits between the two — it is the
    // cheap pointer into `memory_query`, worth more than recent titles.
    let left = left.saturating_sub(omitted_section.len());
    let topics_section = render_brief_topics_section(topics, left);
    let recent_section =
        render_brief_recent_section(recent, left.saturating_sub(topics_section.len()));
    buf.push_str(&omitted_section);
    buf.push_str(&topics_section);
