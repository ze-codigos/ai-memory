...
- `POST /workstream/runs` accepts the caller's `native_session_id`. When a
  ...
  `session_reattached`. An explicit `workstream` still wins. Claude Code
  adoption sends the id on every attempt and keeps a checkout-local
  `adopted-runs/sessions/<id>.json` link that outlives the run, tried first
  by name, so a desktop conversation reopened after a Quit lands in its own
  workstream (the replayed transcript deduplicates by event id) rather than
  fragmenting into `name-<suffix>`. Identity is the native session, never
  the title: a same-title session that is not the same conversation still
  gets its own workstream. (#9)
...
- The Claude Code user-prompt hook renames a launcher placeholder workstream
  ...
  done by hand in the meantime is never overwritten: desktop title when the
  app has one, otherwise a three-word slug of the prompt, both prefixed
...
- ze-codigos fork: the `managed_runs` cancelled-state migration the fork
  shipped as V59 collided with upstream's V59 (`purged_scopes_tombstones`)
  on this merge and now lives at V64. A store migrated by the fork build
  still records it at 59, so the store drops that history row at boot and
  lets refinery replay upstream V59–V63 and re-apply V64; the table rebuild
  it does is idempotent on a table that already carries the CHECK.
